...
def eval_gsf2app(testloader, classes, cnn_model_path, gsf2app_model_path, eval_stage):
	n_classes = len(classes)
	device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

	# Define and Load model
	if eval_stage == 1:
		model = load_pre_trained_model(n_classes, eval_stage, cnn_model_path)
		model.load_state_dict(torch.load(cnn_model_path))
		print("\nEvaluating CNN...")

	elif eval_stage == 2:
		model = GSF2App(cnn_model_path, n_classes)
		model.load_state_dict(torch.load(gsf2app_model_path))
		print("\nEvaluating GSF2App...")
